    ...
    async function load() {
      try {
        ...
        setSettings(data)
         ...
         setStatusMsg({ type: 'error', text: "Failed to load settings! Make sure your database is connected." })
      } finally {
        setIsLoading(false)
      }
    }
  ...
  if (!settings) {
     return (
        <div className="p-8 max-w-2xl mx-auto mt-12 bg-surface rounded-xl shadow-sm border border-red-200">
           ...
           <p className="text-muted">The <code>store_settings</code> table does not exist in your Postgres database yet. Please run your configuration script.</p>
        </div>
     )
  }
