...
export default function PlayerTypeahead({
  ...
  // Keep in sync when an external source changes the value (AI pre-fill, reset, etc.)
  useEffect(() => { setInputText(value) }, [value])

  // Fetch on every debounced keystroke
  useEffect(() => {
    if (debouncedSearch.length < 2) { setOptions([]); return }
    let cancelled = false
    setIsLoading(true)

    fetch(`/api/admin/typeahead?field=player_name&q=${encodeURIComponent(debouncedSearch)}`)
      .then(r => r.json())
